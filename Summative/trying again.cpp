...
void input () {
	...
	if (key[KEY_PLUS_PAD]) {
		if (camera.actualZoom() < maxZoom)
			...
			camera.x += pow (camera.zoom, zoomMagnitude);
			camera.y += pow (camera.zoom, zoomMagnitude);
	}

	if (key[KEY_MINUS] || key[KEY_MINUS_PAD]) {
		camera.zoom -= zoomStep;
		camera.x -= pow (zoomMagnitude, camera.zoom);
		camera.y -= pow (zoomMagnitude, camera.zoom);
	}

	if (key[KEY_Q]) {
		camera.x = planet[EARTH].x;
		camera.y = planet[EARTH].y;
	}
}
...
void entity::detectCollision (struct entity object) {
	...
	float stepDistance = distance (object.x + object.Vx, object.y + object.Vy); //the distance the objects will be at the next move


	if (distance (object.x - object.Vx, object.y - object.Vy) + Vx + Vy <= radius + object.radius) {  //if the entity will touch the earth next move
		Vx = object.Vx;
		Vy = object.Vy;
	}
}
